...
def main():
    ...
    # Read the CSV file
    stations_df = pd.read_csv('data/raw/Bike/cycle_stations.csv')

    # Create a mapping of station names to coordinates from the CSV
    station_coords = {}
    ...
    # Update coordinates in the geocoded stations
    for station_name, station_data in geocoded_data.items():
        # Remove any "_OLD" suffix for matching
        base_name = station_name.split('_OLD')[0]
        
        if base_name in station_coords:
            # Update with coordinates from CSV
            station_data['lat'] = station_coords[base_name]['lat']
            station_data['lon'] = station_coords[base_name]['lon']
            station_data['source'] = 'cycle_stations.csv'
            station_data['display_name'] = f"Updated from cycle_stations.csv"
